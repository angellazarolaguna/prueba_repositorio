...
# ---- bs4 opcional (si no está instalada, ocultamos pestañas de noticias) ----
try:
...
# Google Form (URL termina en /formResponse) para ALTAS
FORM_ACTION_URL = "https://docs.google.com/forms/d/e/1FAIpQLScTbCS0DRON_-aVzdA4y65_18cicMQdLy98uiapoXqc5B6xeQ/formResponse"

# Mapeo columnas -> entry.xxxxx del Form (rellena cuando tengas tus entry reales)
ENTRY_MAP = {
    "Nombre": "",
    "Documento": "",
    "Link": "",
...
# Tema Altair (colores, tipografía, barras redondeadas)
def nfq_theme():
    return {
        "config": {
...
def parse_date(raw):
    # parse manual simple para evitar más dependencias
    if not raw:
        return None
    try:
        # soporta YYYY-MM-DD o DD/MM/YYYY, etc.
...
def site_scraper(label: str, base_url: str, list_url: str | None = None,
                 keywords: list[str] | None = None, must_contain: list[str] | None = None) -> pd.DataFrame:
    """
    - label: nombre de la fuente (p.ej. 'EBA')
    - base_url: URL base
    - list_url: URL de listados (si existe)
        ...
        try:
            ...
            for it in links:
                ...
                if keep and keywords:
                    keep = any(_norm_txt(k) in title_norm for k in keywords)
                if keep:
                    rows.append({
                        "source": label,
                        "title": it["title"],
...
def fetch_all_news(user_keywords: list[str]) -> pd.DataFrame:
    ...
    # NZBA (UNEPFI) — foco: nuevos miembros/movimientos
    ...
    df_eba = site_scraper("EBA",
        ...
        keywords=kws, must_contain=["news", "press", "publication", "esg", "sustainable", "risk"]
    )
    # BIS (ESG)
...
# ===================== RESUMEN "IA local" =====================
STOPWORDS = set("""
a al algo algunas algunos ante antes como con contra cual cuando de del desde donde dos e el ella ellas ellos en entre era erais eramos eran es esa esas ese eso esos esta estaba estabais estabamos estaban estado estais estamos estan estar este esto estos fue fui fuimos fueron ha habeis hemos han hasta hay la las le les lo los mas me mi mis mucho muy nada ni no nos o os para pero poco por porque que quien se sin sobre somos son soy su sus te tiene teneis tenemos tienen tuve tuvo u un una uno y ya the of to in for on at by from as is are was were be been being this that these those with without into through about over after before under again further then once here there when where why how all any both each few more most other some such no nor not only own same so than too very can will just don should now
""".split())

def summarize_text(text: str, max_sentences: int = 6) -> list[str]:
    """Resumen extractivo simple: normaliza, puntúa por frecuencia y devuelve top frases."""
    text = re.sub(r"\s+", " ", text).strip()
    if not text:
        return []
    # Segmentación de frases (heurística)
    ...
    def tokenize(s):
        s_norm = _norm_txt(s)
        toks = re.findall(r"[a-záéíóúñ]+", s_norm)
...
if HAS_BS4:
    ...
    st.warning("Para activar las pestañas de **Noticias** y **Resumen**, añade `beautifulsoup4` a requirements.txt.")
...
if HAS_BS4:
    with tabs[2]:
        ...
        kw_input = st.text_input("Palabras clave (coma separada)",
                                 value=", ".join(DEFAULT_KEYWORDS))
        kws = [k.strip() for k in kw_input.split(",") if k.strip()]
        with st.spinner("Cargando noticias..."):
            news_df = fetch_all_news(kws)

        st.caption("Sugerencia: añade/quita palabras clave para ajustar los resultados.")
        if news_df.empty:
            ...
            with st.expander("Ver en tabla"):
                st.dataframe(news_df[["source","date_str","title","url"]].rename(columns={"date_str":"fecha"}),
                             use_container_width=True)
